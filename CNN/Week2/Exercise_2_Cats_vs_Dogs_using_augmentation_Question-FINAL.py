...
# YOUR CODE STARTS HERE
    ...
    for filename in os.listdir(SOURCE):
        absolute_file = SOURCE + filename
        if os.path.getsize(absolute_file) > 0:
            files.append(filename)
        else:
            print("Empty file: " + filename)
    
    set_size = len(files)
    training_set_size = int(set_size * SPLIT_SIZE)
    testing_set_size  = int(set_size - training_set_size)
